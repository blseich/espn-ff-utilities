...
describe('Best Player', () => {
  ...
  it('should return a player', () => {
    expect(bestPlayer(eligibilePlayerSlotId, playersWithOnlyEligiblePlayer)).toBe(eligiblePlayer);
  });

  it('should return a player with the given eligibility id', () => {
    expect(bestPlayer(eligibilePlayerSlotId, playersWithIneligiblePlayer)).toBe(eligiblePlayer);
  });

  it('should return the player with the heighest score if multiple players have same eligibility id', () => {
    expect(bestPlayer(eligibilePlayerSlotId, playersWithTwoEligiblePlayers)).toBe(eligiblePlayer);
  });
});
